defmodule RumblWeb.VideoControllerTest do
  ...
  describe "With a logged-in user" do
    ...
    test "list all user's videos on index", %{conn: conn, user: user} do
      ...
      other_video = video_fixture(user_fixture(username: "savio"), title: "another video")
      conn = get(conn, Routes.video_path(conn, :index))
      response = html_response(conn, 200)
      assert response =~ ~r/Meus Vídeos/
    ...
    @tag login_as: "leandropnto"
    test "does not create video, renders erros when invalid", %{conn: conn} do
